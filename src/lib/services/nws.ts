...
export async function getLocationByPoint(lat: number, lon: number): Promise<LocationInfo> {
	...
	if (!response.ok) {
		throw createApiError(
			`Points request failed: ${response.status} ${response.statusText}`,
			response.status
		);
	}
...
export async function getStationsByGridpoint(
	gridId: string,
	x: number,
	y: number
): Promise<Station[]> {
	...
	if (!response.ok) {
		throw createApiError(
			`Stations request failed: ${response.status} ${response.statusText}`,
			response.status
		);
	}
...
export async function getLatestObservation(stationId: string): Promise<Observation> {
	...
	if (!response.ok) {
		throw createApiError(
			`Observation request failed: ${response.status} ${response.statusText}`,
			response.status
		);
	}
...
export async function getForecastByUrl(forecastUrl: string): Promise<ForecastDay[]> {
	...
	if (!response.ok) {
		throw createApiError(
			`Forecast request failed: ${response.status} ${response.statusText}`,
			response.status
		);
	}
...
export async function getAlertsByPoint(lat: number, lon: number): Promise<Hazard[]> {
	...
	if (!response.ok) {
		throw createApiError(
			`Alerts request failed: ${response.status} ${response.statusText}`,
			response.status
		);
	}
